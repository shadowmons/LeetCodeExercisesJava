...
public class LeetCodeE {
		...
		public static int JumpGameII(int[] nums) {
			...
			for (int i = 0; i < nums.length -1; i++) {
				potencial = Math.max(potencial,i+ nums[i]);
				if (potencial>=nums.length-1){
					count++;
					break;
		...
		public static void problemaMontyHall () {
			int eleccionInicial= 0;
			double r;
			int contCar =0;
			int contCabra = 0;
			double[] puertas = new double[] {0,0,0};
			int rep = 1500000;
			double porc;
			
			for (int i = 0; i < rep; i++) {
				puertas = new double[] {0,0,0};
				ordenmonty(puertas);
				r = Math.random();
				if(r>0.666) {
					eleccionInicial = 0;
					...
					eleccionInicial = 1;
				}else eleccionInicial = 2;
				
				//System.out.println(puertas[0] + ", " + puertas[1] + ", " + puertas[2]);
				int cabra = 0;
				for (int j = 0; j < puertas.length; j++) {
					
					if (j!=eleccionInicial && cabra==0 && puertas[j]==0) {
						puertas[j]=-1;
						cabra=-1;
						
					}
				}
				//System.out.println(eleccionInicial);
				//System.out.println(puertas[0] + ", " + puertas[1] + ", " + puertas[2]);
			//	System.out.println("");

				
				if(puertas[eleccionInicial] ==1) {
					contCar+=1;
				}
			}
			
			porc = (double) contCar/rep * 100;
			System.out.println("Porcentaje de acierto: " + porc);
		}
		
		public static void ordenmonty(double[] puertas) {
			int car = 0;
			double random = 0;
